...
function BoardEdit() {
	...
	const saveBoard = (e) => {
		e.preventDefault();
		axios.put('/api/boards/update/' + idx, values)
		.then(res => {
			//console.log(2, res);
			navigate('/board');
		})
				...
				<table className="table table-bordered">
					<tbody>
						...
						<tr>
							...
							<td className='col-9'>
								<input className="form-control col-12 w-100" type="text" name='board_title' required placeholder='제목' onChange={ e => setValues({...values, board_title: e.target.value}) } defaultValue={values.board_title}/>
